...
const DIM_LABEL = { quality: 'Chất lượng', growth: 'Tăng trưởng', governance: 'Quản trị', resilience: 'Chống chịu' };
const STATUS_ORDER = ['QUAL', 'MON', 'REV', 'RES', 'EXC'];
const STATUS_LABEL = { QUAL: 'Đủ chuẩn', MON: 'Theo dõi', REV: 'Cần xem lại', RES: 'Thiếu dữ liệu', EXC: 'Loại' };
const MODEL_LABEL = {
...
const MISSING_GOV_LABEL = {
  dilution: 'pha loãng', earnings_cash_gap: 'lệch dòng tiền', late_filing: 'công bố chậm',
};

// Nhãn + kiểu định dạng từng chỉ tiêu. pct: 0,203 → 20,3%; x: 0,63 → 0,63×; years: 0,8 → 4/5 năm
const METRIC = {
...
// Thanh ngắn 0–100, vạch = ngưỡng Đủ chuẩn; dưới ngưỡng "Cần xem lại" thì tô khác.
function dimBar(it, d) {
  const dim = it.dims?.[d] || {};
  const q = qualifyOf(d), rv = reviewOf(d);
  ...
  return `<div class="wl-bar" role="img" aria-label="${DIM_LABEL[d]} ${Math.round(s)}, ngưỡng Đủ chuẩn ${q}">
...
function select(ticker) {
  ...
  const dimSections = ['quality', 'growth', 'resilience'].map(d => {
    const dim = it.dims?.[d];
    const rows = (spec[d] || []).map(m => {
      const pct = it.percentiles?.[m.key];
      return `<tr class="${it.metrics?.[m.key] == null ? 'wl-miss' : ''}">
        <td>${esc((METRIC[m.key] || [m.key])[0])}<small> ${m.higher_better ? '↑' : '↓'} ${m.weight}%</small></td>
        <td class="td-num">${fmtMetric(m.key, it.metrics?.[m.key])}</td>
    ...
    return `<div class="detail-section">
      ...
      ${rows ? `<table class="wl-metrics"><thead><tr><th>Chỉ tiêu</th><th class="th-num">Giá trị</th><th class="th-num">Percentile</th></tr></thead><tbody>${rows}</tbody></table>`
             : '<p class="muted">Mô hình ngành chưa kích hoạt.</p>'}
    </div>`;
  }).join('');

  const gov = it.dims?.governance || {};
  const flags = (it.governance_flags || []).map(f => `<li>${esc(FLAG_LABEL[f] || f)} <small>−${(state.meta.governance_penalty || {})[f] ?? ''}</small></li>`).join('');
  ...
  box.innerHTML = `
    ...
    <div class="detail-section">
      ...
      ${flags ? `<ul class="wl-flags">${flags}</ul>` : '<p class="muted">Không có cờ.</p>'}
      ${govMissing ? `<p class="muted">Chưa đánh giá được: ${esc(govMissing)}</p>` : ''}
