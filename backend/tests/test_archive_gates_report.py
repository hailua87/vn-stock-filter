...
def test_both_gates_always_reported():
    cases = [(at(23, 12), summary(0.98)),
             ...
             (at(12, 0), summary(1.0))]
    for now, fs in cases:
        names = [g['name'] for g in evaluate_archive_gates(now, fs)]
...
def test_passing_gate_is_reported_not_omitted():
    ...
    gates = evaluate_archive_gates(at(23, 12), summary(0.62, 'time_budget'))
...
def test_both_gates_can_fail_at_once():
    ...
    """
    d = archive_decision(now=at(12, 52), fetch_summary=summary(0.62, 'time_budget'))
...
def test_passing_coverage_detail_says_so():
    detail = gate(evaluate_archive_gates(at(23, 12), summary(0.98)),
...
def test_unmeasured_coverage_is_not_dressed_up_as_pass():
    ...
    """
    g = gate(evaluate_archive_gates(at(23, 12), None), 'gate_coverage')
...
def test_custom_threshold_shows_in_detail():
    g = gate(evaluate_archive_gates(at(23, 12), summary(0.62), min_coverage=0.5),
             'gate_coverage')
    assert g['passed'] is True
...
def test_truncated_but_above_threshold_still_fails():
    ...
    g = gate(evaluate_archive_gates(at(23, 12), summary(0.95, 'circuit_breaker')),
             'gate_coverage')
...
def test_metadata_carries_structured_gates():
    ...
    d = archive_decision(now=at(23, 12), fetch_summary=fs)
...
def test_force_names_the_gates_it_overrode():
    d = archive_decision(force=True, now=at(12, 52),
                         fetch_summary=summary(0.62, 'time_budget'))
...
def test_force_on_a_clean_run_is_not_marked_forced():
    d = archive_decision(force=True, now=at(23, 12), fetch_summary=summary(1.0))
...
def test_decision_table_unchanged_by_the_rewrite(hour, minute, cov, stop,
    ...
    d = archive_decision(force=force, now=at(hour, minute),
                         fetch_summary=summary(cov, stop))
