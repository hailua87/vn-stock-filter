...
def check_prices() -> None:
    ...
    df = vci.ohlcv('FPT', str(start), str(end))
    ...
    idx = vci.ohlcv('VNINDEX', str(start), str(end))
...
def check_overview() -> None:
    ...
    row = vci.company_overview('VCB') or {}
...
def check_statements() -> None:
    ...
    for path in sorted(FIXTURES.glob('vnstock407_*.json')):
        ...
        for table in TABLES:
            live = vci.financial_statement(ticker, table, period=period)
            label = f'{ticker} {period} {table}'
            if live.empty:
                check(False, label, 'rỗng')
                continue
            cols = fx[table]['columns']
            fx_periods = cols[3:]
            live_ids = set(live['item_id'])
            missing = sorted({r[2] for r in fx[table]['data']} - live_ids)
            check(not missing, f'{label}: đủ item_id', f'thiếu {missing[:5]}' if missing else '')

            # Ô so sánh: (item_id, kỳ) có ở cả hai phía. Lấy dòng ĐẦU của mỗi
            # item_id như statement_to_records.
            first_live = live.drop_duplicates('item_id').set_index('item_id')
            total = bad = 0
            for row in fx[table]['data']:
                item_id = row[2]
                if item_id not in first_live.index:
                    continue
                for p, v in zip(fx_periods, row[3:]):
                    if p not in first_live.columns or v is None:
                        continue
                    lv = first_live.at[item_id, p]
                    total += 1
                    expect = float(lv) * FIXTURE_SCALE
                    if abs(expect - float(v)) > max(1.0, abs(float(v)) * 1e-6):
                        bad += 1
            shared = [p for p in fx_periods if p in first_live.columns]
            check(len(shared) >= min(4, len(fx_periods)), f'{label}: nhãn kỳ khớp',
                  f'chung {len(shared)}/{len(fx_periods)} kỳ, live có {list(first_live.columns[2:7])}')
